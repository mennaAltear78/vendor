...
function Menue({
  ...
  return (
    <div className={`relative  items-center font-usedFont ${table?'display flex':''}`} >
      ...
      <div
        className={`bg-white  border border-solid border-[#56595c44] rounded-lg px-[6px] py-0 cursor-pointer flex justify-between items-center text-[16px] text-gray-500 transition-[width] duration-300 ease-in-out ${
          table ? "w-[100px] h-[30px]" : "w-[190px] h-[30px]"
        }`}
      >
        <div className="display flex items-center " onClick={toggleDropdown}>
          <div className={table ? "w-[85px]" : "w-[174px]"}>
            {table ? (
              <p className="text-gray-500">{labelMenue} </p>
            ) : selectedOption ? (
              `${selectedOption} `
            ) : null}
            {!NObtn && TimeOption}
